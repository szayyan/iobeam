...
use crate::{
    buffer_pool::{BUFFER_SIZE, BufferPool},
...
impl<'a> UringSubmitter<'a> {
    ...
    unsafe fn queue_send(&mut self, fd: Fd, buf: *const u8, len: usize, user_data: u64) {
    ...
    unsafe fn queue_multishot_accept(&mut self, fd: Fd, user_data: u64) {
    ...
    unsafe fn queue_recv(&mut self, fd: Fd, buf: *mut u8, len: usize, user_data: u64) {
    ...
    unsafe fn queue_close(&mut self, fd: Fd, user_data: u64) {
    ...
    unsafe fn sync_sq_and_empty_backlog(&mut self) -> io::Result<()> {
        loop {
            if self.sq.is_full() {
...
impl<'a> UringServer<'a> {
    pub fn new(
        ...
        Self {
            listener,
            file_system_handler,
            sub: UringSubmitter::new(submitter, sq),
            cq,
            buffer_pool,
            token_alloc,
        }
    ...
    pub fn start_event_loop(&mut self) -> anyhow::Result<()> {
        unsafe {
            self.prep_initial_accept();
        }

        loop {
            self.sub.submit_and_wait()?; // NOTE erroring here may leave fd's open leaking memory
            self.cq.sync();
            unsafe {
                self.sub.sync_sq_and_empty_backlog()?;
            }
                ...
                match token.clone() {
                    ...
                    Token::Read {
                        ...
                        if ret == 0 {
                            ...
                            *token = Token::Close;
                            unsafe {
                                self.sub.queue_close(Fd(fd), token_index as _);
                            }
                        } else {
                                    ...
                                    if !is_get_request(&request) {
                                            ...
                                            .open_raw_fd("./public/index.html")
                                        {
                                            ...
                                            Err(e) if e.kind() == ErrorKind::NotFound => {
                                                response_header =
                                                    Some(HTTP1_CONTENT_NOT_FOUND_ERROR)
                                            }
                                            Err(e) => {
                                                response_header = Some(HTTP1_INTERNAL_SERVER_ERROR)
                                            }
                                        }
                                    }
                                }
                    ...
                    Token::WriteHeaders {
                        ...
                        if write_complete {
                            ...
                            if let Some(body) = body {
                                ...
                                while remaining > 0 {
                                    let n =
                                        unsafe { libc::sendfile(fd, body.fd, &mut off, remaining) };
                                    if n <= 0 {
                                        break;
                                    }
    ...
    unsafe fn prep_initial_accept(&mut self) {
