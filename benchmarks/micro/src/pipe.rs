...
fn bench_pipe(c: &mut Criterion) {
    ...
    for &calls_per_loop in PARAM_VALUES {
        group.bench_with_input(
            BenchmarkId::new("libc", calls_per_loop),
            &calls_per_loop,
            |b, &n| {
                b.iter(|| {
                    for _ in 0..n {
        ...
        let mut ring = IoUring::new(calls_per_loop as u32 * 2).unwrap();

        group.bench_with_input(
            BenchmarkId::new("uring", calls_per_loop),
            &calls_per_loop,
            |b, &n| {
                b.iter(|| {
